...
export const getBooks = catchAsync(async (req: Request, res: Response, next: NextFunction): Promise<void> => {
...
export const getBookById = catchAsync(async (req: Request, res: Response, next: NextFunction): Promise<void> => {
...
export const updateBook = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const updatedBook: IBook | null = await Book.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
    })
    if (!updatedBook) {
